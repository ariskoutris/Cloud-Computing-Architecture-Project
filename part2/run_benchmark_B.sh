...
for PARSEC_JOB in "${PARSEC_JOBS[@]}"; do
    ...
    for n_threads in 1 2 4 8; do
        ...
        envsubst < $TEMPLATE_FILE | kubectl create -f -
        ...
        kubectl get jobs
        (kubectl logs $(kubectl get pods --selector=job-name=$PARSEC_JOB --output=jsonpath='{.items[*].metadata.name}')) > results/b/${PARSEC_JOB}_${n_threads}.txt
